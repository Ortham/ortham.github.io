...
Replace the `--fs-uuid` value with your EFI partition's UUID, and the EFI filename with the filename of the UKI generated by `ukify`. You can find your EFI partition's UUID by looking for its entry in the output of `ls -l /dev/disk/by-uuid/`.
